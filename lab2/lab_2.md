...
- `^` — начало слова,
- `$` — конец слова.   **(2) Замена$cc^*$ на $c^+$.**   В блоке$bcc^*a$ подвыражение $cc^*$ задаёт одну или более букв `c`.
Следовательно:
...
сохраняет язык:
- добавление `^` и `$` фиксирует сопоставление всей строки целиком; - замена$cc^*$ на $c^+$эквивалентна, так как обе конструкции задают одну или более букв `c`; - вынесение общего префикса `b` в альтернативе$bbb \mid bc$ сохраняет язык.
