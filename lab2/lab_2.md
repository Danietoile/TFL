...
```math
R' = \textasciicircum\Bigl( b\bigl( a \mid (c(b(bb \mid c))^* a (bc^{+} a)^* \mid ac )^* \bigr)ac \Bigr)^* \textdollar
```
...
- замена $cc^*$ на $c^+$ эквивалентна, так как обе конструкции задают одну или более букв `c`;
